...
import { Check, ChevronRight } from 'lucide-static';
...
class DropdownMenuGroup extends Component<DropdownMenuGroupSignature> {
  ...
  register = modifier(() => {
    const unregister = this.menuContext.registerGroupCloseCallback?.(this.closeAllSubmenus);
...
class DropdownMenuRadioGroup extends Component<DropdownMenuRadioGroupSignature> {
  ...
  <template>
    <div class={{cn @class}} data-slot='dropdown-menu-radio-group' role='radiogroup' ...attributes>
...
class DropdownMenuSubTrigger extends Component<DropdownMenuSubTriggerSignature> {
  ...
  <template>
    ...
    >
      {{yield}}
      <Icon @svg={{ChevronRight}} class='ml-auto size-4' />
    </div>
  </template>
}
...
class DropdownMenuSubContent extends Component<DropdownMenuSubContentSignature> {
  ...
  positionSubmenu = modifier(
    (element: HTMLElement, [triggerElement]: [HTMLElement | null | undefined]) => {
